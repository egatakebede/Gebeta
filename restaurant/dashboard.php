...
<script>
    ...
    function goTab(tab) {
        ...
        // Always guard: if JS function or DOM is missing, don't break other navigation.
        try { if (tab === 'orders' && typeof loadOrders === 'function') loadOrders(false); } catch(e) { console.error('loadOrders failed', e); }
        try { if (tab === 'menu' && typeof loadMenu === 'function') loadMenu(false); } catch(e) { console.error('loadMenu failed', e); }
        try { if (tab === 'analytics' && typeof loadAnalytics === 'function') loadAnalytics(false); } catch(e) { console.error('loadAnalytics failed', e); }
        try { if (tab === 'customers' && typeof loadCustomers === 'function') loadCustomers(false); } catch(e) { console.error('loadCustomers failed', e); }
        try { if (tab === 'finance' && typeof loadFinance === 'function') loadFinance(false); } catch(e) { console.error('loadFinance failed', e); }
    ...
    async function loadDashboardData() {
        ...
        try {
            ...
            try {
                ...
                throw new Error('Invalid JSON response from /api/dashboard-data.php');
            }
            ...
            if (errorEl && errorTextEl) {
                errorTextEl.textContent = e?.message ? e.message : String(e);
